...
function computeHunkDiffs(path, before, after) {
	...
	for (const hunk of patch.hunks) {
		...
		diffs.push({
			...
			// 1-based first line of this hunk on each side — lets UIs render a
			// Claude Code style unified diff with real line numbers.
...
function applyWriteTool(ctx, sandbox) {
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			presentationMeta: (args, value) => ({ diffs: value.before === null ? [] : computeHunkDiffs(args.file_path, value.before, value.after).map(({ path, oldText, newText, oldStart, newStart }) => ({
				path,
				oldText,
				newText,
				oldStart,
...
function applyEditTool(ctx, sandbox) {
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			presentationMeta: (args, value) => ({ diffs: computeHunkDiffs(args.file_path, value.before, value.after).map(({ path, oldText, newText, oldStart, newStart }) => ({
				path,
				oldText,
				newText,
				oldStart,
