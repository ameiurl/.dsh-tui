...
import { t, getLang, setLang, isLang, writeLangPref, readLangPref, subscribeLang, LANGS } from '../i18n.js';
...
function clonePermissionPresetSnapshot(snapshot) {
    return {
...
export function Chat({ channel, questionStore, approvalStore, extensionDialogs, extensionStatus, extensionShortcuts, themeHost, onExit, onUpdate, onRestart, fullscreen = false, trajectorySeen: trajectorySeenProp, injectControllerRef, }) {
    ...
    React.useEffect(() => {
        if (extensionShortcuts === undefined)
            return;
     ...
     *  翻转；落定后自动回到普通行的折叠语义。 */
    const [streamViewToggledRows, setStreamViewToggledRows] = React.useState(() => new Set());
    /**
    ...
    const [modelRecents, setModelRecents] = React.useState(() => readModelRecents());
    ...
    const [modelGroup, setModelGroup] = React.useState(undefined);
    ...
    const modelGroups = React.useMemo(() => deriveModelGroups(models, providerInfos, modelRecents), [models, providerInfos, modelRecents]);
    ...
    const groupModels = React.useMemo(() => {
        if (activeModelGroup === undefined)
            return [];
        if (activeModelGroup === RECENTS_GROUP_PROVIDER)
            return recentCatalogModels(modelRecents, models);
    ...
    const switchModelRecorded = (provider, id, name) => {
        if (name !== undefined)
            channel.notify(t('model-switching', { name }));
        return channel.switchModel(provider, id).then((ok) => {
            if (!ok)
                return ok;
            if (name !== undefined)
                channel.notify(t('model-switched', { name }));
    ...
    const recapAbortRef = React.useRef(null);
    const closeRecap = () => {
    ...
    React.useEffect(() => {
        ...
        setRecap(null);
        if (!channel.autoRecapOnOpen)
            return;
        // No conversation yet (/new): nothing to recap, don't even fire.
        if (!channel.rows.some(row => row.kind === 'user' || row.kind === 'assistant'))
            return;
        recapAbortRef.current?.abort();
    ...
    const openFileActions = React.useCallback((rawPath) => {
    ...
    const runFileAction = React.useCallback((index, path) => {
        if (index === 0)
            openFile(path);
        else if (index === 1)
    ...
    React.useEffect(() => {
        const ink = instances.get(process.stdout) ?? instances.values().next().value;
        if (ink)
            ink.onHyperlinkClick = handleOpenTarget;
        return () => {
     ...
     */
    const runWorkspaceMenuOption = (option) => {
        ...
        switch (name) {
            ...
            case 'color': {
                ...
                if (!isValidSessionColor(colorName)) {
                    channel.notify(t('color-unknown', { name: colorName, list: SESSION_COLOR_NAMES.join(' · ') }), { color: 'error' });
                    return true;
                }
                ...
                if (parts.length > 0) {
                    ...
                    void channel.listSkills().then((list) => {
                        ...
                        if (skill.userInvocable)
                            ...
                            channel.notify(t('skills-not-invocable', { name: parts[0] }), { color: 'warning' });
                    });
                    return true;
                }
                setHelpOpen(false);
     ...
     */
    const requestRewindConfirm = async (row) => {
        ...
        if (token !== rewindRequestRef.current)
            return;
        if (decision === 'cancel') {
    ...
    const toggleStreamView = React.useCallback((rowId) => {
    ...
    useInput((input, key, event) => {
        ...
        if (sceneOpen || channel.pluginScene !== undefined)
            return;
        ...
        if (key.wheelUp || key.wheelDown) {
            ...
            const overlayModal = overlay.kind !== 'none' &&
    ...
    if (pluginScene !== undefined) {
        const node = (_jsx(PluginSceneBoundary, { id: pluginScene.id, onError: (id, error) => {
                channel.notify(t('plugin-scene-crashed', { id, err: error.message }), { color: 'error' });
                channel.closePluginScene();
            }, children: React.createElement(pluginScene.component, {
                React,
                ui: tuiKit,
                channel,
    ...
    if (settingsOpen) {
        const screen = _jsx(Settings, { channel: channel, onClose: () => setSettingsOpen(false) });
        return fullscreen ? screen : _jsx(AlternateScreen, { children: screen });
    }
    ...
    if (subagentDetailId !== null) {
        ...
        const scene = (_jsx(SubagentDetailScene, { subagent: subagent, onInterrupt: (id) => channel.subagentControl.interrupt(id), onBack: () => {
                setSubagentDetailId(null);
                setSubagentDashboardOpen(true);
            } }));
    ...
    if (jobsPanelOpen) {
        ...
        return fullscreen ? panel : _jsx(AlternateScreen, { children: panel });
    }
    // Subagent dashboard: displays all active and completed subagents.
    // Like the browser and settings, it replaces the conversation entirely.
    if (subagentDashboardOpen) {
        const dashboard = (_jsx(SubagentDashboard, { subagents: [...channel.subagents], onSelect: (id) => {
                setSubagentDashboardOpen(false);
                setSubagentDetailId(id);
            }, onClose: () => setSubagentDashboardOpen(false) }));
        return fullscreen ? dashboard : _jsx(AlternateScreen, { children: dashboard });
    }
    ...
    return (_jsxs(Box, { ref: wakeTickRef, flexDirection: "column", flexGrow: 1, width: "100%", children: [!isSticky && anchorUserText && (_jsx(PinnedTurnHeader, { text: anchorUserText, onClick: () => {
                    ...
                    if (turn)
                        handle?.scrollTo(turn.top);
                    else if (anchorUserRowId !== null)
                        ...
                        (channel.activityEnabled &&
                            ...
                            channel.workingActivity.phase !== 'idle' ? (_jsx(Box, { marginTop: 1, children: _jsx(ActivityLine, { activity: channel.workingActivity, activityFrames: channel.activityFrames, warnPct: activityWarnPct, warnDanger: activityWarnPct !== undefined && activityWarnPct >= 95, 
                                ...
                                channel.agentViewRows !== undefined && backgroundAgentsNeedingInput > 0
                                    ? backgroundAgentsNeedingInput
                                    : undefined, controllerRef: promptControllerRef, onCaretImage: handleCaretImage, caretPreviewOpen: peekPreview !== null, onDismissCaretPreview: dismissPeek, onVimChange: (enabled, insert) => setVimMode({ enabled, insert }) }, "prompt-input"), _jsx(StatusLine, { channel: channel, selectionActive: selectionActive, helpOpen: helpOpen, vim: vimMode, wake: wakeBand === undefined
                                    ? undefined
                                    : {
                                        band: wakeBand,
