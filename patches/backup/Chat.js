...
export function Chat({ channel, questionStore, approvalStore, extensionDialogs, extensionStatus, extensionShortcuts, themeHost, onExit, onUpdate, onRestart, fullscreen = false, trajectorySeen: trajectorySeenProp, injectControllerRef, renderScene, }) {
    ...
    useInput((input, key, event) => {
        ...
        if (actionMatches('history', input, key) && !helpOpen) {
            // Ctrl+R searches the same directory-scoped history ↑/↓ walks:
            // `loadHistory(cwd)` returns this working directory's entries.
...
function PinnedTurnHeader({ text, onClick, }) {
    ...
    return (_jsx(Box, { flexShrink: 0, width: "100%", height: 1, overflow: "hidden", paddingRight: 1, onClick: onClick, children: _jsx(Text, { color: "userPromptLabel", bold: true, wrap: "truncate-end", children: label }) }));
}
