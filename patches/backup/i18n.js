...
const dict = {
    ...
    // F2: the directory rail, the workspace page it drilled into and the
    // right-click menu are not part of this fork, so no key that drives them is
    // advertised here. The rest of the stock keys (Tab preview, branch/run
    // filters, rename, delete, clean, pin) still work.
    'session-hint-list': { zh: '**Enter** 恢复 · Tab 预览 · {{mod}}s 子运行（{{runs}}） · {{mod}}b 本分支 · {{mod}}r 重命名 · {{mod}}p 固定 · {{mod}}d 删除 · {{mod}}x 清空壳 · Esc 退出', en: '**Enter** resume · Tab preview · {{mod}}s runs ({{runs}}) · {{mod}}b this branch · {{mod}}r rename · {{mod}}p pin · {{mod}}d delete · {{mod}}x clean · Esc exit' },
    'session-hint-list-mid': { zh: '**Enter** 恢复 · Tab 预览 · {{mod}}s 子运行 · {{mod}}r 重命名 · {{mod}}p 固定 · {{mod}}d 删除 · Esc 退出', en: '**Enter** resume · Tab preview · {{mod}}s runs · {{mod}}r rename · {{mod}}p pin · {{mod}}d delete · Esc exit' },
