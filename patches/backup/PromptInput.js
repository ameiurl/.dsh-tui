...
function vimWordForward(text, cursor) {
...
function vimWordBackward(text, cursor) {
    let i = cursor;
...
function boundaryAtOrBefore(bounds, offset) {
    let lo = 0;
...
function previousGraphemeBoundary(bounds, offset) {
    let lo = 0;
    let hi = bounds.length - 1;
    let ans = 0;
    while (lo <= hi) {
...
function nextGraphemeBoundary(bounds, offset) {
    let lo = 0;
    let hi = bounds.length - 1;
    let ans = bounds[hi] ?? 0;
    while (lo <= hi) {
...
function normalizeCursorOffset(text, offset) {
...
export function PromptInput({ channel, suspended = false, helpOpen, onToggleHelp, onRunCommand, selectionActive, fillText, onFillConsumed, onRewindRequest, onBackgroundRequest, backgroundAgentsNeedingInput, controllerRef, onCaretImage, caretPreviewOpen = false, onDismissCaretPreview, }) {
     ...
     */
    const [foldBlock, setFoldBlock] = React.useState(null);
    /** Synchronous mirror used by batched keys, controller clear, and mouse drag. */
    const foldBlockRef = React.useRef(null);
    /**
    ...
    const editorViewportRef = React.useRef(null);
    ...
    const hoverLeaveTimerRef = React.useRef(null);
    ...
    const handleEnter = () => {
        ...
        if (channel.working && value.trim() !== '') {
            ...
            if (parsed !== undefined && (((parsed.name === 'btw' || parsed.name === 'skills')
    ...
    useInput((input, key, event) => {
        ...
        const clampRowStart = (pos) => block && cursor >= block.end ? Math.max(pos, block.end) : pos;
        const clampRowEnd = (pos) => block && cursor <= block.start ? Math.min(pos, block.start) : pos;
        if (block) {
            if (key.escape) {
                updateFoldBlock(null);
                return;
        ...
        if (input.includes('\n') || input.includes('\r')) {
            if (expandedRef.current) {
                insertAtCaret(sanitizeEditableText(input.replace(/\r\n/g, '\n').replace(/\r/g, '\n')));
                return;
            }
            if (/^[\r\n]+$/.test(input)) {
                handleEnter();
        ...
        if (key.upArrow) {
            ...
            if (block && cursor >= block.end) {
                ...
                if (line > 0) {
                    ...
                    const prevLineStart = upToLineStart === -1 ? 0 : tail.lastIndexOf('\n', upToLineStart - 1) + 1;
                    const prevLine = tail.slice(prevLineStart, upToLineStart);
                    setInput(value, block.end + prevLineStart + Math.min(cursorColumn(tail, tailCursor), prevLine.length));
                    return;
                }
                setInput(value, block.start);
                return;
        ...
        if (key.downArrow) {
            ...
            if (block && cursor >= block.end) {
                ...
                if (line < lines.length - 1) {
                    ...
                    const nextLine = tail.slice(nextLineStart, nextLineEnd === -1 ? tail.length : nextLineEnd);
                    setInput(value, block.end + nextLineStart + Math.min(cursorColumn(tail, tailCursor), nextLine.length));
                    return;
                }
            }
                ...
                if (line < lines.length - 1) {
                    ...
                    const nextLine = head.slice(nextLineStart, nextLineEnd === -1 ? head.length : nextLineEnd);
                    setInput(value, nextLineStart + Math.min(cursorColumn(head, cursor), nextLine.length));
                    return;
                }
                setInput(value, block.end);
                return;
        ...
        const vimNormalEdit = (next, cursorOffset) => {
            updateFoldBlock(null);
        ...
        if (vimEnabledRef.current) {
            if (vimInsertRef.current) {
                ...
                const plainChar = input.length > 0 &&
        ...
        if (key.escape) {
            ...
            channel.notify(value.length === 0 ? t('esc-again-rewind') : t('esc-again-clear'));
    ...
    const caretVisualLine = block && cursor > block.start
    ...
    if (expanded) {
        ...
        if (!editorFreeScrollRef.current) {
            if (caretVisualLine < win)
                win = caretVisualLine;
            if (caretVisualLine >= win + editorMaxRows)
                win = caretVisualLine - editorMaxRows + 1;
        }
        ...
        windowStart = Math.max(0, Math.min(caretVisualLine - MAX_VISIBLE_LINES + 1, visualLines.length - MAX_VISIBLE_LINES));
        visibleCount = MAX_VISIBLE_LINES;
    }
    const visibleLines = visualLines.slice(windowStart, windowStart + visibleCount);
    ...
    const foldPreviewWidth = inputWidth - stringWidth(foldBadge) - stringWidth(` · ${foldHint}`) - 6;
    const foldPreview = foldPreviewWidth >= 8
    ...
    const prefixCols = !block && !expanded && big && windowStart === 0 ? stringWidth(prefixLabel) : 0;
    ...
    const rendered = visibleLines.map((line, index) => {
        ...
        if (block && absoluteLine === chipRow) {
            ...
            return (_jsxs(Box, { flexDirection: "row", onClick: (event) => {
                    event.stopImmediatePropagation();
                    updateFoldBlock(null);
                }, onMouseEnter: hoverEnter, onMouseLeave: hoverLeave, children: [_jsx(Text, { dimColor: true, children: foldBadge }), foldPreview !== '' && _jsx(Text, { dimColor: true, children: " \u00B7 " }), foldPreview !== '' && _jsx(Text, { wrap: "truncate-end", children: foldPreview }), _jsx(Text, { dimColor: true, children: ` · ${foldHint}` })] }, `fold-${absoluteLine}`));
        }
    ...
    const editorRows = expanded
        ? visibleLines.map((line, index) => {
            ...
            const gutterLabel = logicalNo === undefined
    ...
    const valueBoxRef = useDeclaredCursor({
        ...
        column: Math.min(caretVisualCol +
            (expanded
                ? editorGutterCols + 1
     ...
     * not to text).
     */
    const localToOffset = (localCol, localRow) => {
    ...
    const handleValueClick = (e, colOffset = 0) => {
        ...
        const col = clamped === 0 && prefixCols > 0 ? Math.max(0, localCol - prefixCols) : localCol;
        const offset = clickToCursorOffset(value, clamped === 0 && prefixCols > 0 ? inputWidth - prefixCols : inputWidth, clamped, col, isDouble ? 'grapheme-start' : 'nearest');
        if (isDouble) {
     ...
     */
    const handleDragStart = (e, colOffset = 0) => {
    ...
    React.useLayoutEffect(() => {
        if (floatersOpen === prevFloatersOpenRef.current)
            return;
    ...
    const editorNode = editorVisible ? (_jsxs(Box, { flexDirection: "column", width: "100%", height: "100%", borderStyle: "round", borderColor: promptAccent, backgroundColor: "toolCardBackground", children: [_jsxs(Box, { flexDirection: "row", flexShrink: 0, paddingLeft: 1, paddingRight: 1, children: [_jsx(Text, { bold: true, color: promptAccent, children: `✎ ${t('input-expand-editor-title')}` }), _jsx(Box, { flexGrow: 1 }), _jsx(Text, { dimColor: true, children: t('input-fold-stats', {
                ...
                }, onDragEnd: handleDragEnd, onWheel: (event) => {
    ...
    return (_jsxs(Box, { flexDirection: "column", marginTop: 1, children: [floatersOpen && (_jsxs(OverlayAbove, { maxHeight: Math.max(terminalRows - 6, 1), children: [helpOpen && (_jsx(Box, { marginBottom: 1, children: _jsx(HelpMenu, { commands: channel.commandList, viewportHeight: helpViewportHeight, viewportWidth: columns, scrollRef: helpScrollRef, onCommandPick: (name) => {
                                ...
                                setInput(`/${name} `);
                                onToggleHelp();
                            } }) })), !helpOpen && channel.pending.length > 0 && (_jsxs(Box, { flexDirection: "column", paddingLeft: 2, paddingBottom: 1, children: [channel.pending.some(item => item.placement === 'steer') && (_jsxs(Box, { flexDirection: "column", children: [_jsxs(Text, { dimColor: true, children: ["\u26A1 ", t('input-pending-steer-label')] }), channel.pending
                                        ...
                                        .map(item => (_jsxs(Text, { dimColor: true, wrap: "truncate", children: ['  ', "\u21B3 ", item.text] }, item.id)))] })), _jsxs(Text, { dimColor: true, children: ["Alt+\u2191 ", t('input-pending-actions-hint')] })] })), fileOverlayOpen && (_jsx(FileSuggestions, { files: fileMatches, selectedIndex: fileSelected, columns: columns, query: mention?.query ?? '', accent: promptAccent, 
                        ...
                        } })), peekOpen && (_jsx(Box, { onMouseEnter: hoverEnter, onMouseLeave: hoverLeave, children: _jsx(SuggestionCard, { title: stats, columns: columns, accent: promptAccent, footer: peekClipped
 ...
 */
function wrapLineRows(line, width) {
    if (line === '')
...
function wrapToWidth(text, width) {
    return text.split('\n').flatMap(line => wrapLineRows(line, width).map(r => line.slice(r.start, r.end)));
}
/**
 ...
 */
function caretInText(text, width, offset) {
    let line = 0;
    let lineBase = 0;
 ...
 */
function clickToCursorOffset(text, width, visualLine, visualCol, snapWithin = 'nearest') {
    const segmenter = getGraphemeSegmenter();
    let row = 0;
    let lineBase = 0;
        ...
        for (const r of wrapLineRows(line, width)) {
            if (row === visualLine) {
                ...
                for (const { segment } of segmenter.segment(line.slice(r.start, r.end))) {
                    ...
                    if (currentWidth + w / 2 > visualCol)
                        return lineBase + r.start + local;
                    if (currentWidth + w > visualCol)
                        return lineBase + r.start + local + segment.length;
                    currentWidth += w;
 ...
 */
function visualLineRanges(text, width) {
    const ranges = [];
    let lineBase = 0;
    for (const line of text.split('\n')) {
        for (const r of wrapLineRows(line, width))
            ranges.push([lineBase + r.start, lineBase + r.end]);
        lineBase += line.length + 1;
    }
    return ranges;
}
 ...
 */
function wordSelectionAt(text, offset, lo, hi) {
    if (offset < lo || offset >= hi)
        return null;
    const bounds = graphemeBoundaries(text.slice(lo, hi)).map(b => b + lo);
    let i = 0;
    while (i < bounds.length - 1 && bounds[i + 1] <= offset)
        i++;
    const clusterAt = (index) => text.slice(bounds[index], bounds[index + 1]);
    const cls = selectionCharClass(clusterAt(i));
    let a = i;
    while (a > 0 && selectionCharClass(clusterAt(a - 1)) === cls)
        a--;
    let b = i;
    while (b < bounds.length - 2 && selectionCharClass(clusterAt(b + 1)) === cls)
        b++;
