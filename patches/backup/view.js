...
export function normalizeWorkspaceCwd(cwd) {
...
function hiddenLineageIds(sessions, currentId) {
 ...
 * avoided.
 */
export function buildWorkspaceGroups(sessions, context) {
    const hidden = hiddenLineageIds(sessions, context.currentId);
    const conversations = sessions.filter(session => session.hasPrompt && session.kind.kind !== 'subagent' && !hidden.has(session.id));
 ...
 */
export function buildView(sessions, filters, context, pinned = EMPTY_PINS) {
    ...
    const inScope = (session) => !hiddenLineage.has(session.id) &&
    ...
    let shown = 0;
    let lastProjectKey;
    const visibleChildren = (session) => filters.showSubagents
    ...
    if (pinnedRows.length > 0) {
        ...
        for (const session of pinnedRows)
            emit(session);
    }
    ordered.sort((left, right) => right.updatedAt - left.updatedAt);
    const projectKey = (session) => context.sameProject(context.cwd, session.cwd)
        ? 'current'
        : `cwd:${normalizeWorkspaceCwd(session.cwd) || '<unknown>'}`;
    const projectDisplay = new Map();
    ...
    if (filters.allProjects) {
        ...
        for (const session of ordered) {
            ...
            if (!projectOrder.has(key))
                projectOrder.set(key, projectOrder.size);
